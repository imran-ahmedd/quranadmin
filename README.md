...
- **ইউজার ম্যানেজমেন্ট** — সব ইউজার সার্চ করে দেখা, প্রতি ইউজারের প্রগ্রেস
  ...
  মুছে ফেলা।
- **মডারেশন (নতুন)** — প্রতি ইউজারের ড্রয়ারে এখন একটা "অ্যাকাউন্ট
  নিয়ন্ত্রণ" প্যানেল আছে:
  - **স্ট্যাটাস** — সক্রিয় / সীমিত / ব্লক, সাথে একটা এডমিন-নোট (reason)।
    সীমিত হলে ইউজার লগইন করতে পারবে কিন্তু নাম/পদবি/বায়ো পরিবর্তন করতে
    পারবে না; ব্লক হলে পরের বার অ্যাপ খুললেই স্বয়ংক্রিয়ভাবে সাইন-আউট
    হয়ে যাবে। মূল AlQuran অ্যাপের `js/auth.js` এই `status` ফিল্ড পড়ে
    enforce করে (এই রিপো শুধু ফিল্ড লেখে)।
  - **কাস্টম থিম গ্রান্ট** — একটা টগল দিয়ে যে কাউকে কাস্টম থিম বিল্ডার
    সরাসরি আনলক করে দেওয়া যায় (`customThemeGranted: true`), সাধারণত যেটা
    ৩০-দিনের স্ট্রিকে আনলক হয়। মূল অ্যাপের `js/theme-builder.js`-এ এই
    ফ্ল্যাগ চেক করা হয়।
  - **প্রোফাইল টেক্সট ক্লিয়ার** — নাম/পদবি/বায়োতে অনুপযুক্ত কিছু লিখলে
    সেই একটা ফিল্ডই খালি করে দেওয়া যায়, বাকি ডকুমেন্ট অক্ষত থাকে।
  - ইউজার লিস্টে স্ট্যাটাস অনুযায়ী ফিল্টার করা যায় (সব/সক্রিয়/সীমিত/ব্লক)।
- **স্টোরি মডারেশন (নতুন)** — WhatsApp-স্টাইল স্ট্যাটাস/স্টোরি ফিচারে
  (js/status.js) কে কী পোস্ট করছে, সব লাইভ (এখনো এক্সপায়ার হয়নি এমন)
  স্টোরি রিয়েল-টাইমে এখানে দেখা যায় — টেক্সট/আয়াত টাইপ অনুযায়ী ফিল্টার,
  এক ক্লিকে স্টোরি মুছে ফেলা, আর "ইউজার মডারেট করুন" বাটনে সরাসরি সেই
  ইউজারের ড্রয়ারে (স্ট্যাটাস/থিম কন্ট্রোলসহ) চলে যাওয়া যায়। সাইডবারে
  একটা লাইভ কাউন্ট ব্যাজ থাকে।
  - "সীমিত"/"ব্লক" করা অ্যাকাউন্ট নতুন স্টোরি পোস্টই করতে পারবে না — এটা
    Firestore rules-এ সার্ভার-সাইডে বাধ্যতামূলক করা আছে (শুধু ক্লায়েন্ট-সাইড
    চেক না, যা ডেভটুলস দিয়ে বাইপাস করা যেত)।
...
service cloud.firestore {
  match /databases/{database}/documents {
    ...
    match /users/{uid} {
      ...
      allow read, update, delete: if isAdmin(); // এডমিন প্যানেলের জন্য — আপডেট লাগবে মডারেশন (status/customThemeGranted/name-position-bio ক্লিয়ার) ফিচারের জন্য
